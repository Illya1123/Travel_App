  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const user = {
        ...
        phone: result.userData.mobile,
        address: result.userData.address,
        birthday: result.userData.dateOfBirth,
      }
      ...
      showToast('error', 'Lỗi đăng nhập', error.response?.data?.message || 'Đăng nhập thất bại. Vui lòng thử lại.')
    }
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-white px-8">
      <View className="flex-1 items-center justify-center">
        <Image source={require('../../assets/images/logo.jpg')} className="w-40 h-40 mb-6" />
        <Text className="text-3xl font-bold mb-6 text-center">Đăng nhập</Text>
        ...
        <TouchableOpacity className="w-full bg-blue-500 p-4 rounded-lg mb-4" onPress={handleSignIn}>
          <Text className="text-white text-center font-semibold text-lg">Đăng nhập</Text>
        </TouchableOpacity>

        <Text className="text-center">
          Bạn chưa có tài khoản?{' '}
          <Text className="text-blue-500 font-semibold" onPress={() => router.push('/auth/signUp')}>
            Đăng ký
          </Text>
        </Text>
        ...
        <Text className="text-center">
          Bạn quên mật khẩu?{' '}
          <Text className="text-blue-500 font-semibold" onPress={() => router.push('/auth/forgotPassword')}>
            Đặt lại mật khẩu
          </Text>
        </Text>
      </View>
